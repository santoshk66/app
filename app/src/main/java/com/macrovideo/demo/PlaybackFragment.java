...
public class PlaybackFragment extends Fragment implements OnClickListener, OnItemClickListener {
    ...
    Map<Integer, HSFileDownloader> mRecFileDownloaderMap;
    private List<Integer> mRecFileDownloaderList;

    private HSFileDownloader mRecFileDownloader;
            ...
            if (msg.arg1 == Defines.HANDLE_MSG_CODE_GET_RECORD_FILES_END) {
                ...
                switch (msg.arg2) {
                    ...
                    case ResultCode.RESULT_CODE_SERVER_CONNECT:
                        ...
                        break;
                    case ResultCode.RESULT_REC_SEARCH_NONE:
                        if (isActive) {
                            try {
                                toast = Toast.makeText(relateAtivity.getApplicationContext(), getString(R.string.noticRecOKNOFiles), Toast.LENGTH_SHORT);
                                toast.setGravity(Gravity.CENTER, 0, 0);
                        ...
                        break;
                    case ResultCode.RESULT_REC_SEARCH_NO_CARD:
                        if (isActive) {
                            try {
                                toast = Toast.makeText(relateAtivity.getApplicationContext(), getString(R.string.lblNOSDCard), Toast.LENGTH_SHORT);
                                toast.setGravity(Gravity.CENTER, 0, 0);
                                toast.show();
                            } catch (Exception e) {
                                e.printStackTrace();
    ...
    private class RecFileListViewAdapter extends BaseAdapter {
        ...
        private void downloadFile(final int position) {
            ...
            if (isStorageEnough) {
                ...
                final HSFileDownloader recFileDownloader = new HSFileDownloader();
